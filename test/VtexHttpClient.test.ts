...
describe("VTEXHttpClient tests", () => {
  ...
  test("performRequest, with get method, performs the request", () =>
    new Promise((done) => {
      ...
      const scope: nock.Scope = nock(BASE_URL)
        ...
        .replyWithFile(
          200,
          getFilepath("vtexResponses/listOrders.json"),
  ...
  test("performRequest, with post method with body, performs the request", () =>
    new Promise((done) => {
      const path: string = "/api/oms/pvt/orders/orderId/changes";
      const method: string = "POST";
      const requestBody: any = JSON.parse(
        readFileSync(
          getFilepath("vtexRequests/registerChangeOnOrder.json"),
          "utf-8"
        )
      );
      const scope: nock.Scope = nock(BASE_URL)
  ...
  test("performRequest, with post method with body and additional headers, performs the request", () =>
    new Promise((done) => {
      ...
      vtexHttpClient
        .performRequest(path, method, requestBody, additionalHeaders)
        .then(() => {
          expect(scope.isDone()).toBe(true);
          done();
        });
  ...
  test("performRequest, with post method with body that returns 400, performs the request", () =>
    new Promise((done) => {
      const path: string = "/api/oms/pvt/orders/orderId/changes";
      const method: string = "POST";
      const requestBody: any = JSON.parse(
      ...
      vtexHttpClient.performRequest(path, method, requestBody).catch(() => {
        expect(scope.isDone()).toBe(true);
        done();
      });
    }));
